...
\date{2026-09-19 \quad (technical report, version 2)}
...
\begin{center}
\fbox{\parbox{0.92\textwidth}{\small
\textbf{What changed since version 1 (2026-09-05).} Version 1 described the
...
budget, calibrating the two tuning constants, and the fast kernels that set the step time)
...
effect. A preliminary Chinese cell suggests that the apparent language effect is a
training-mass effect.
\end{abstract}
...
\paragraph{Cost.} On an AMD Ryzen AI MAX+ 395 with 128\,GB of unified memory, one descent
on one hundred facts runs 280--330 steps of one 2\,048-token pack each, at about 24\,s per
step in the captured phase and 29\,s in the free phase, evaluations included, or about
2.4 hours; the
engine measurements take another two. The step is bound by memory bandwidth. These times
use fast kernels for the expert layers that are not released; the released reference path
runs the same descent more slowly, with bit-identical expert logits and a difference of
about $2\times10^{-3}$ in the loss from compiled fusions. The overlay for one hundred facts
is a 9\,MB file.
...
\paragraph{Composition.} Of 83 probes asking for two facts in one answer, the released
seed-1 cell produces both answers in 10 (9 clean, 1 followed by a hallucination), the
captured cells in 3 and 5, the base model in 0. Composition remains rare and seed
dependent: the method stores literal memory with a trigger, and does not by itself teach
the model to combine what it stored.
...
\section{A second language (preliminary)}
\label{sec:lang}

We rebuilt the same world in Chinese, with the same recipe, row budget and seed (the
Chinese cell is preliminary; English is in preparation). Measured raw, the Chinese cell is
weaker: 0.676 exact match on 954 test fragments against 0.841 for Italian, with base models
at 0.003 and 0.005. The picture changes with the training mass. The row budget, tuned on
...
bootstrap over facts): at equal mass the two languages are indistinguishable. The contest
...
Italian overlay), and affects Chinese text more (0.0166). This suggests that a graft lives in
the rows keyed by the tokens of its own script, and that a fact taught in one language will
not be recalled in another. Both readings are hypotheses: the decisive tests, a Chinese cell
given the same mass per fact as the Italian one, and the Italian test set run on the Chinese
overlay, have not been run.
...
two seeds, and a preliminary second language. The method is meant for every model with an
...
facts and should be traded against recall by whoever deploys it.
...
and of the preliminary Chinese cell; and the configuration of the measured run. The
reference path uses a basic expert kernel; the measured runs used faster kernels that are
not released, with bit-identical expert logits. The pipeline that produced the corpus and
